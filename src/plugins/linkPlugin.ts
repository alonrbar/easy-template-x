...
export class LinkPlugin extends TemplatePlugin {
    ...
    public async simpleTagReplacements(
        tag: Tag,
        data: ScopeData,
        ...
        const relId = await context.docx.rels.add(
            content.target,
            LinkPlugin.linkRelType,
            linkAttributes
        );

        // generate markup
        const wordRunNode = this.utilities.docxParser.containingRunNode(
            wordTextNode
        );
    ...
    private generateMarkup(
        ...
        const runProps = wordRunNode.childNodes.find(
            node => node.nodeName === DocxParser.RUN_PROPERTIES_NODE
        );
        if (runProps) {
    ...
    private insertHyperlinkNode(
        ...
        // the run.
        let textNodesInRun = tagRunNode.childNodes.filter(
            node => node.nodeName === DocxParser.TEXT_NODE
        );
        if (textNodesInRun.length > 1) {
            const [runBeforeTag] = XmlNode.splitByChild(
                tagRunNode,
                tagTextNode,
                true
            );
            textNodesInRun = runBeforeTag.childNodes.filter(
                (node: XmlNode) => node.nodeName === DocxParser.TEXT_NODE
            );
