...
export const XmlNode = {
      ...
      if (domNode.nodeType === domNode.ELEMENT_NODE) {
        ...
        if (attributes) {
          ...
          for (let i = 0; i < attributes.length; i++) {
            ...
            (xmlNode as XmlGeneralNode).attributes[curAttribute.name] =
              curAttribute.value;
          }
        }
      }
  ...
  isTextNode(node: XmlNode): node is XmlTextNode {
    if (
      ...
      if (
        !(
          node.nodeType === XmlNodeType.Text && node.nodeName === TEXT_NODE_NAME
        )
      ) {
        throw new Error(
  ...
  insertBefore(newNode: XmlNode, referenceNode: XmlNode): void {
    ...
    if (!referenceNode) throw new MissingArgumentError(nameof(referenceNode));
...
function getDescendantPath(root: XmlNode, descendant: XmlNode): number[] {
  ...
  while (node !== root) {
    ...
    if (!parent)
      throw new Error(
        `Argument ${nameof(descendant)} is not a descendant of ${nameof(root)}`
      );
